...
void findSCC(const graph& g, vector<vector<int>>& SCCarr)
{
	SCCarr.clear();
	bool* visited = new bool[g.size()];
	for (int i = g.size() - 1; i >= 0; i--)
...
void getSCCGraph(const graph& g, graph& SCCGraph, const vector<vector<int>>& SCCarr)
{
	...
	for (int i = SCCarr.size() - 1; i >= 0; i--)
	{
		...
		for (int j = SCCarr[i].size() - 1; j >= 0; j--)
		{
			...
			{
				if (!edges[SCCBelonging[neighbours[k]]])
				{
...
void edgeBFS(const graph& g, int startIndex, const TVisit& visit, TCollector& collector)
{
	bool* visited = new bool[g.size()];
	for (int i = g.size() - 1; i >= 0; i--)
	{
...
void visit(int index, const graph& g, vector<bool>& arr)
{
	arr[index] = true;
}

void getTransitiveClosure(const graph& g, vector<vector<bool>>& closure)
{
	closure.clear();
	closure = vector<vector<bool>>(g.size(), vector<bool>(g.size(), false));
	for (int i = 0; i < g.size(); i++)
	{
		edgeBFS(g, i, visit, closure[i]);
	}
}

void purdom(const graph& g, vector<vector<bool>>& closure)
{
	vector<vector<int>> SCCarr;
	findSCC(g, SCCarr);
	graph SCCGraph;
	getSCCGraph(g, SCCGraph, SCCarr);
	closure.clear();
	closure = vector<vector<bool>>(g.size(), vector<bool>(g.size(), false));
	vector<vector<bool>> SCCClosure;
	getTransitiveClosure(SCCGraph, SCCClosure);
	for (int i = 0; i < SCCarr.size(); i++)
	{
		for (int j = 0; j < SCCarr.size(); j++)
		{
			if (SCCClosure[i][j])
			{
				for (int k = SCCarr[i].size() - 1; k >= 0; k--)
				{
					for (int t = SCCarr[j].size() - 1; t >= 0; t--)
					{
						closure[SCCarr[i][k]][SCCarr[j][t]] = true;
					}
				}
			}
		}
